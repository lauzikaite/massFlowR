...
groupPEAKS(files = faahKO_files, out_dir = out_directory, cwt = cwt_param, ncores = 2)
```
...
For each PCS, a set of representative peaks, which detection in samples is consistent, are identified. Intensity values of these peaks are then reported for each sample. For samples in which representative peaks are not present, intensity data is re-integrated using original LC-MS files. In contrast to `xcms` package, *m/z* and *rt* values for intensity integration are estimated for each sample separetely. *m/z* and *rt* values are taken from two surrounding samples, in which the peak-of-interest was present, and extrapolated. 

# Peak annotation

*(under development)*

If in-house chemical reference database is available, PCS are annotated using representative peaks. For more details how to build a database file, see [annotation using database](http://htmlpreview.github.io/?https://github.com/lauzikaite/massFlowR/blob/master/inst/doc/annotation.html))).
