...
validPEAKS_paral <-
  function(pkg,
    ...
    peaks_ids <-
      unique(pkg_ints$peakid) ## extract peak-ids for the peak-group
    pkg_cor <-
      setNames(as.data.frame(t(combn(
        peaks_ids, 2, simplify = T
      ))),  ## make peak-peak pairs
      nm = c("from", "to")) ## column names "from" and "to" will be needed in graph generation
    
    min_samples_n <-
      ceiling(length(unique(pkg_ints$run_order)) * (min_samples / 100))
...
extractINT <- function(pkg, object) {
  ...
  pkg_ints <- lapply(names(object@data), function(s) {
    ...
    sample <-
      setNames(sample[which(sample$tmp_peakid %in% pkg_peakids), c("tmp_peakid", "into")], c("peakid", "into"))
    sample$filepath <- rep(s, nrow(sample))
    sample$run_order <-
      rep(object@samples[which(object@samples == s), "run_order"], nrow(sample))
    return(sample)
  })
  pkg_ints <-
    do.call(function(...)
      rbind(..., make.row.names = F), pkg_ints)
...
corPEAKS <- function(pair, pkg_ints, min_samples_n) {
  ...
  if (length(common_samples) > min_samples_n) {
    ...
    cc <-
      ifelse(cc < 0, 0, cc) ## negative coeficients would break graph generation
  } else {
    cc <- 0
  }
  return(cc)
